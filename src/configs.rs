...
use pprzlink::parser::{PprzProtocolVersion, PprzDictionary, PprzMessage, PprzMsgBaseType,
                       build_dictionary, PprzMsgClassID};
...
pub fn link_build_dictionary(config: Arc<LinkConfig>) -> Arc<PprzDictionary> {
    ...
    let xml_file =
        config.pprz_root.clone() + "/sw/ext/pprzlink/message_definitions/v1.0/messages.xml";
...
pub fn link_init_and_configure() -> Arc<LinkConfig> {
    ...
    let pprzlink_version = match pprzlink_version as u32 {
        1 => PprzProtocolVersion::ProtocolV1,
        2 => PprzProtocolVersion::ProtocolV2,
        _ => {
...
pub fn link_update_status(
    ...
    let dt_seconds =
        status_msg_period.as_secs() as f32 + (status_msg_period.subsec_nanos() as f32 * 1e-9);
    ...
    for field in &mut msg.fields {
        match field.name.as_ref() {
            ...
            "rx_bytes_rate" => {
                let byte_rate =
                    (status_report.rx_bytes - status_report.last_rx_bytes) as f32 / dt_seconds;
                field.value = PprzMsgBaseType::Float(byte_rate);
            }
            "rx_msgs_rate" => {
                let byte_rate =
                    (status_report.rx_msgs - status_report.last_rx_msgs) as f32 / dt_seconds;
                field.value = PprzMsgBaseType::Float(byte_rate);
            }
            "tx_msgs" => {
