...
impl LinkComm {
        ...
        if config.udp {
            ...
            let mut com = LinkComm {
                ...
                remote_addr: (config.remote_addr.clone() + ":"
                    + &config.udp_uplink_port.to_string())
                    .parse()?,
            };
            ...
            socket
                .set_read_timeout(Some(Duration::new(0, 1_000_000)))
                .expect("set_read_timeout call failed");
            // set write timeout of 10^6 ns (1ms)
            socket
                .set_write_timeout(Some(Duration::new(0, 1_000_000)))
    ...
    pub fn com_write(&mut self, buf: &[u8]) -> Result<usize, IOError> {
        match self.com_type {
            LinkCommType::Serial => match self.port {
                Some(ref mut port) => port.write(buf),
                None => Err(IOError::new(IOErrorKind::Other, "Port not initialized")),
            },
            LinkCommType::Udp => match self.socket {
                Some(ref mut socket) => socket.send_to(buf, self.remote_addr),
                None => Err(IOError::new(
                    IOErrorKind::Other,
                    "Tx socket not initialized",
                )),
            },
        }
    }
